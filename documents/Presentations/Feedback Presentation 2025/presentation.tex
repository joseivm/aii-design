...
\begin{frame}{Quick Updates}
    \begin{itemize}
        ...
        \item I ran the analysis with the Thai data and got positive results for both the single zone and multi zone case.
        \item I added a proof that the multi-zone problem is a relaxation of the single zone problem. 
        \item I talked to XY, Rad, and a potential 4th committee member. 
    \end{itemize}
\end{frame}
...
\begin{frame}{Multiple Zone Model}
    \begin{align}
        \max_{a,b,K,\pi} &\quad \mathbb{E}\left [ \sum_z s_z U\left ( w_{0,z} + 1 -\ell_z^j -\pi_z + I_z(\theta^j_z) \right ) \right ]\nonumber\\
...
\begin{frame}{Single-zone vs Multi-zone}
  \begin{itemize}
    ...
    \item We will show that $P2$ is a relaxation of running $P1$ for all zones and taking the sum. I.e. $P2^* \geq \sum_z P1(z)^*$.
  \end{itemize}
\end{frame}
...
\begin{frame}{SZ vs MZ Conclusion}
    $\ell_z, \hat{\ell_z}, I_z(\hat{\ell_z})$ and $w_0$ are the same in both objectives. The only difference between the objectives of $P2$ and $P1$ is $\pi^{MZ}$ and $\pi^{SZ}$. Since $u()$ is monotonically increasing, we have that $P2 \geq \sum_z P1(z)$
...
\begin{frame}{Perfromance Metrics}
    \begin{itemize}
        \item We focus on the Relative Insurance Benefit (RIB) as our main metric. 
        \item Used in index insurance studies (\cite{kenduiywo2021evaluating}; \cite{estefania2024breaking})
...
\begin{frame}{RIB Definition}
    ...
    \begin{itemize}
        \setlength\itemsep{1em}
        \item CE(J): $u^{-1}(\mathbb{E}[u(w^J)])$ 
        \item $\Delta CE(J): = CE(J) - CE(NI)$
        \item RIB(J): $\frac{\Delta CE(J)}{\Delta CE(PI)}$
...
\begin{frame}{Single Zone Results}
    Here, the contract design and pricing for each zone happens independently of all other zones. Premium pricing is also independent: $\pi_z = \bbe [I_z(\theta_z)]+c_k K_z$, with $K_z = {\sf CVaR}_{1-\epsilon_K}\left ( I_z(\theta_z)\right )  - \mathbb{E}[I_z(\theta_z)] $
    \begin{table}[]
        \centering
        \begin{tabular}{lllll}
...
\begin{frame}{MZ vs SZ Case}
    In MZ case premium is based on portfolio required capital: $K = {\sf CVaR}_{1-\epsilon_K} \left ( \sum_z s_z\overline{I_z(\theta_z)} \right ) - \mathbb{E}\left [ \sum_{z} s_{z}\underline{I_{z}(\theta_{z'})} \right ]$. 
...
\begin{frame}{Insurance Behavior}
    \begin{itemize}
        \setlength\itemsep{2em}
        \item Payout Precision: share of all payouts that are "valid". $P = \frac{\sum_{i: \hat{y_i} > 0} w_i \hat{y_i} \min \{\ell_i, \hat{y_i}\}}{\sum_{i: \hat{y_i} > 0} w_i \hat{y_i}}$
        \item Loss Recall: ratio of valid payouts to loss. $R =  \frac{\sum_{i: \hat{y_i} > 0} w_i \hat{y_i} \min\{\ell_i, \hat{y_i}\}}{\sum_{i: \hat{y_i} > 0} w_i \ell_i}$
    \end{itemize}
    
\end{frame}

\begin{frame}{Valid Gains and Bad Losses}
    \begin{table}[]
        \centering
        \begin{tabular}{lll}
                                     & Utility Gain           & Utility Loss           \\ \cline{2-3} 
        \multicolumn{1}{l|}{Loss}    & \multicolumn{1}{l|}{A} & \multicolumn{1}{l|}{B} \\ \cline{2-3} 
        \multicolumn{1}{l|}{No Loss} & \multicolumn{1}{l|}{C} & \multicolumn{1}{l|}{D} \\ \cline{2-3} 
        \end{tabular}
        \end{table}

        A: utility gain during loss $\rightarrow$ "valid gain"\\
        B: utility loss during loss $\rightarrow$ "bad loss"
\end{frame}
...
\begin{frame}{Insurance Behavior Results}
    \begin{table}[]
        ...
        Method    & RIB    & Payout Precision & Loss Recall & Valid Gain & Bad Loss \\ \hline
        VMX-M     & 0.592  & 0.184            & 0.211       & 0.390          & 0.334        \\
        VMX       & 0.524  & 0.207            & 0.128       & 0.379          & 0.431        \\
        Chantarat & -0.163 & 0.338            & 0.021       & 0.594          & 0.465        \\
        Chen      & -1.909 & 0.173            & 0.298       & 0.329          & 0.440       
        \end{tabular}
        \end{table}
\end{frame}
...
\begin{frame}{Summary of results}
    \begin{itemize}
        \item Our method leads to larger utility gains.
        \item Has more efficient behavior. 
        \item Lower cost.
    \end{itemize}
\end{frame}
