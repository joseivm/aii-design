...
\section{Optimization Approach}
  ...
  %   Model motivation: objective and constraints discussed above. Short discussion of idealized model.
  %   \subsubsection{Single Zone}
  %     Parameters + Model
  %   \subsubsection{Multiple Zone}
  %     Parameters + Model
  %   \subsubsection{Challenges/Shortcomings}
  %     Probabilistic constraints non-convex (add evidence). Only way for it to be convex is if $I(\theta)$ is linear, but those aren't the functions we want to opt over. 

    % The first model we proposed aimed to minimize the probability that a farmer's net loss would exceed an exogenously given threshold. This was subject to a maximum premium constraint and a constraint specifying a piecewise linear structure for the payout function. Index insurance payout functions generally have the form: $f(x) = \min \left \{(a\theta +b)^+,K \right \}$, where $K$ is the maximum payout amount. In the model below, $\ell$ is the farmer's loss, $\pi$ is the premium he pays, $I(\theta)$ is the insurance payout, $c_k$ is the cost of capital, and $\bar{\ell}$ is the loss threshold we don't want the farmer to exceed. In other words, we want the farmer to have net losses lower than $\bar{l}$ with high probability. The premium contains two terms, the first is for the expected payout of the contract, and the second is for the costs of holding the capital necessary to insure the contract. $K^P$ is the amount of capital needed by the insurer to be able to uphold their side of the contract with high probability. For exposition, we first show the optimization model for insuring a single zone, and we then show the model for insuring multiple zones. 
    % TODO: add what epsilon is in this model. 
  ...
  % \subsubsection{Motivation}
  %   We want a model that minimizes the probability that wealth drops below a certain threshold, subject to a budget constraint. This leads us to the following idealized model. In the model, $\ell$ is the realization of the loss, $\pi$ is the premium paid, $I(\theta)$ is the payout function, $\bar{\ell}$ is the critical loss threshold, $\epsilon$ is bla, $K^P$ is the amount of required capital, and $K$ is the maximum payout. 

  %   \begin{align}
  %     \min_{a,b,\pi}  &\quad P(\ell -I(\theta) \geq \bar{\ell})\\
  %     \text{s.t.   } & I(\theta) = \min \{ (a\hat{\ell}(\theta) + b)^+,K \} \\
  %     I(\theta) &+ c_k K^P \leq B\\
  %     K^P &= CVaR_{1-\epsilon}\left ( I(\theta) \right ) - E[I(\theta)]
  % \end{align}
...
% In order for the whole program to be convex, $\epsilon$ must be fixed. However, we can set it based on the desired payout frequency (verify this) or on the types of losses that are most relevant in a certain setting. This can be determined with input from stakeholders. \\
% The use of $CVaR$ as an objective can also be motivated by its desireable properties as a risk measure. In this setting, by minimizing the $CVaR$ of the net loss, we are minimizing the expected value of the net loss, conditional on it being above a certain quantile. Intuitively, this makes sure that even losses in the right tail of the distribution are not too large. $CVaR$ also has the advantage of having tractable reformulations, both when used as an objective and as a constraint. This leads us to the following model: 

% \begin{align}
%   \min_{a,b,\pi}  & \quad CVaR_{1-\epsilon}\left ( \ell - I(\theta) \right)\\
%   \text{s.t.   }I(\theta) &= \min \{ (a\hat{\ell}(\theta) + b)^+,K \} \\
%   I(\theta) &+ c_k \left( CVaR_{1-\epsilon}\left ( I(\theta) \right ) - E[I(\theta)] \right) \leq B \label{cons-budget}
  % \pi &= E[I(\theta)]
% \end{align}
