...
PREDICTIONS_DIR = os.path.join(EXPERIMENTS_DIR,f"prediction{suffix}")
...
##### Contract Design #####
def optimization_program_new(simulated_preds: np.ndarray, sample_df: pd.DataFrame, params: dict, eval_set='Val'):
    """
    Now: premium per zone = E_sample[alpha_obj] + (c_k/sumS)*K
    (alpha_obj is the lower-spliced payout on sample_df for that zone, weighted by w)
    The capital K and CVaR block still use simulated_preds as before.
    """
    # Unpack params
    ...
    # ----------------------------------------------------------------------------
    # 1) SIMULATED DATA BLOCK (premium & CVaR resources computed on simulations)
    # ----------------------------------------------------------------------------
    ...
    alpha_sim = cp.Variable((n_sim, Z))   # lower splice on sims
    omega_sim = cp.Variable((n_sim, Z))   # upper splice on sims
    ...
    # Splicing on simulations (unchanged)
    constraints += [
    ...
    # Keep basic shape constraints
    constraints += [
    a >= 0,                    # payout increases with predicted loss
    a <= 10.0,                 # heuristic upper bound; tune to your scale
    b >= 0,
    b <= max_payout,           # intercept not wildly large

    pi >= 0,                   # premiums nonnegative
    K  >= 0,                   # capital nonnegative
    ]

    # ----------------------------------------------------------------------------
    # 2) OBJECTIVE BLOCK (sample_df with CARA utility)
    #    + Build alpha_obj/omega_obj so we can define premiums from the *sample*
    # ----------------------------------------------------------------------------
    alpha_obj = {}
    omega_obj = {}
    ...
    obj_terms = []
    expected_alpha_sample_list = []   # will hold E_sample[alpha_obj[z]] per zone
    ...
    for iz, z in enumerate(zones):
        ...
        # weighted zone contribution
        sum_wz    = w_z.sum()
        zone_term = zone_sizes[iz] * (w_z @ util_z) / sum_wz
        obj_terms.append(zone_term)

        # store the sample-weighted expected lower-splice payout for premium
        expected_alpha_sample = (w_z @ alpha_obj[z]) / sum_wz
        expected_alpha_sample_list.append(expected_alpha_sample)

    # ----------------------------------------------------------------------------
    # 2b) PREMIUM DEFINITION FROM SAMPLE + CAPITAL CHARGE (moved here)
    # ----------------------------------------------------------------------------
    # pi == E_sample[alpha_obj] + (c_k/sumS)*K
    constraints += [
        pi == cp.hstack(expected_alpha_sample_list) + (c_k/sumS) * K,
    ...
    if eval_set == 'Val':
        ...
        solver_outcome = None
        try:
            solver_outcome = prob.solve(
                solver=cp.ECOS,      # more room than default
                abstol=1e-8, reltol=1e-7, feastol=1e-8,
                verbose=False
            )
            
            
        except cp.SolverError:
            # ECOS failed? try SCS (supports exp cone; slower but robust)
            solver_outcome = prob.solve(
                solver=cp.SCS,
                max_iters=40000,
                eps=1e-5,              # default 1e-4; slightly tighter
                acceleration_lookback=20,
                verbose=False           # print diagnostics if you want
            )
        

    # NOTE: preserving the original return signature (first zone only).
    # Consider returning the full vectors: `return a.value, b.value`
    return a.value[0], b.value[0]

def optimization_program(simulated_preds: np.ndarray, sample_df: pd.DataFrame, params: dict, eval_set='Val'):
...
def run_eval(state, length, params, model_name, eval_set='Test'):
    ...
    if eval_set == 'Test':
        ...
        train_df = model_preds.loc[model_preds.Set == 'Train',:]
        test_df = model_preds.loc[model_preds.Set == 'Val',:]
        # train_df, test_df = train_test_split(train_df, test_size=0.5, random_state=2)

    sim_preds = simulate_single_zone_payouts(train_df, state, n_sim=n_sim)
    sim_matrix = sim_preds['PredLoss'].to_numpy().reshape(-1,1)
    # sim_matrix = train_df['PredLoss'].to_numpy().reshape(-1,1)
    # print(f"Max sim payout: {sim_preds['PredLoss'].max()}")
    # Design contracts
    params['P'] = np.round(train_df['Loss'].max(),0)
    # print(f"Max train Loss: {params['P']}")
    start = time.time()
    a, b = optimization_program(sim_matrix, train_df, params, eval_set)
    end = time.time()
    ...
    # opt_test_premium, required_capital = calculate_sz_premium(opt_test_payouts, params['c_k'], sim_train_payouts)
    ...
    results['Method'] = 'VMX'
    results['Required Capital'] = required_capital/opt_premium
...
def run_chen_eval(state, length,params):
    ...
    chen_metrics['Required Capital'] = req_capital/chen_premium
...
def calculate_performance_metrics(payout_df, params):
    ...
    w_0, alpha = params['w_0'], params['risk_coef']
    pdf = payout_df.copy()
    pdf['NI_Wealth'] = w_0 - pdf['Loss']
    pdf['NI_Utility'] = -(1/alpha)*np.exp(-alpha*pdf['NI_Wealth'])
    pdf['UtilityDiff'] = pdf['Utility'] - pdf['NI_Utility']

    # pdf['PosLoss'] = pdf.Loss > 0
    # pdf['UtilityGain'] = pdf.UtilityDiff > 0
    # gdf = pdf.groupby(['PosLoss','UtilityGain'])['UtilityDiff'].sum()
    ...
    return {
        ...
        'PredLoss': pdf['PredLoss'].mean(),
        'Loss': pdf['Loss'].mean(),
        'Premium': pdf['Premium'].mean(),
        'Insurer Cost': pdf['Payout'].mean()
    }
...
def choose_best_model(state, length, params):
    ...
    results_fname = os.path.join(pred_dir,f"results_{length}.csv")
    rdf = pd.read_csv(results_fname)
    bad_model_dict = {str(i):[] for i in range(20,81)}
    bad_model_dict['83'] = []
    # bad_models = []
