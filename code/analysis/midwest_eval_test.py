...
# see why it's slightly smaller in the multi state case for IL. 
...
##### Testing #####
# 1. Make sure we are using the same data
def check_same_data(states):
    lengths = [i*10 for i in range(2,9)]
    for length in lengths:
        odf = load_all_model_predictions(states, length)
        cdf = load_all_chen_payouts(states, length)
        print(f"#### Length {length} ####")
        print(f"Illinois: {np.allclose(odf.loc[odf.State == 'Illinois','Loss'],cdf.loc[cdf.State == 'Illinois','Loss'])}")
        print(f"Indiana: {np.allclose(odf.loc[odf.State == 'Indiana','Loss'],cdf.loc[cdf.State == 'Indiana','Loss'])}")
        print(f"Iowa: {np.allclose(odf.loc[odf.State == 'Iowa','Loss'],cdf.loc[cdf.State == 'Iowa','Loss'])}")
        print(f"Missouri: {np.allclose(odf.loc[odf.State == 'Missouri','Loss'],cdf.loc[cdf.State == 'Missouri','Loss'])} \n")

def check_chen_data(states):
    lengths = [i*10 for i in range(2,9)]
    for length in lengths:
        print(f"#### Length {length} ####")
        cdf = load_all_chen_payouts(states, length)
        for state in states:
            sdf = load_chen_payouts(state,length,1)
            print(f"{state}: {np.allclose(cdf.loc[cdf.State == state,'Payout'],sdf['Payout'])}")
...
def run_chen_eval(states, length, params):
    chen_payouts = load_all_chen_pyaouts(states, length)
